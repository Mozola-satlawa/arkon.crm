...
export async function handler(event) {
  ...
  try {
    // GET /api/messages?room=<id>&before=<iso>
    if (event.httpMethod === "GET") {
      ...
      const { blobs } = await store.list({ prefix }); // do 1000 szt.
      const items = [];

      for (const b of blobs || []) {
        const obj = await store.get(b.key, { type: "json" });
        if (!obj) continue;
        if (before && (obj.created_at || "") >= before) continue; // starsze niż "before"
        items.push(obj);
      }
    ...
    // POST /api/messages
    // body: { room, author, body, fileUrl?, parentId? }
    if (event.httpMethod === "POST") {
      ...
      // obsługujemy 2 tryby: przesłany fileUrl lub body będące URLem
    ...
    if (event.httpMethod === "PATCH") {
      ...
      if (patch.reaction) {
        ...
        data.reactions[patch.reaction] = (data.reactions[patch.reaction] || 0) + 1;
      }
    ...
    // DELETE /api/messages?id=<id>  (miękkie usunięcie)
