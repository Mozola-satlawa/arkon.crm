...
export async function handler(event) {
  ...
  try {
    // Netlify daje nam body (często base64). Zbudujemy z niego Request, żeby użyć FormData.
    const ct =
    ...
    const key = `uploads/${room}/${Date.now()}-${crypto
      .randomUUID()
      .slice(0, 8)}.${ext}`;
    ...
    // publiczny URL Netlify Blobs
    const url = `/.netlify/blobs/${encodeURIComponent(
      "chat-uploads"
